...
function obtenerExtension(file: File): string | null {
  if (file.type && ALLOWED_TYPES_BY_MIME[file.type]) {
    return ALLOWED_TYPES_BY_MIME[file.type]
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const file of files) {
      ...
      // Generate a clean filename for disk storage
    ...
    return NextResponse.json({
      archivos: uploadedResults,
      totalProcesados: uploadedResults.length,
      exitosos: uploadedResults.filter((r) => !r.error).length,
    })
  } catch (error: any) {
